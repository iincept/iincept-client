...
// Default iPad AppleCare rows fallback
const DEFAULT_IPAD_APPLECARE_ROWS = [
  { 
...
const IPAD_SUB_NAV_ITEMS = [
  { name: 'iPad Pro', query: 'iPad Pro', image: '/ipad_nav/ipad_pro.png', scale: 'scale-100' },
  { name: 'iPad Air', query: 'iPad Air', image: '/ipad_nav/ipad_air.png', scale: 'scale-100' },
  { name: 'iPad', query: 'iPad', image: '/ipad_nav/ipad.png', scale: 'scale-100' },
  { name: 'iPad mini', query: 'iPad mini', image: '/ipad_nav/ipad_mini.png', scale: 'scale-95' },
  { name: 'AppleCare+', path: '/ipad?tab=applecare', image: '/applecare_official_hero.png', scale: 'scale-100' },
  { name: 'Apple Pencil', query: 'Pencil', image: '/ipad_nav/apple_pencil.png', scale: 'scale-95' },
  { name: 'Keyboards', query: 'Keyboard', image: '/ipad_nav/keyboards.png', scale: 'scale-95' },
  { name: 'Compare', path: '/compare', image: '/ipad_nav/ipad_compare.png', scale: 'scale-100' }
];
...
const resolveIconImage = (img, label) => {
  ...
  const matched = IPAD_SUB_NAV_ITEMS.find(m => m.name.toLowerCase().trim() === lblLower);
  return matched?.image || '/ipad_nav/ipad.png';
};
...
export default function Ipad() {
  ...
  const [subItems, setSubItems] = useState(() => getInitialSubItems('ipad', IPAD_SUB_NAV_ITEMS));
  const [localWishlist, setLocalWishlist] = useState({});
  const [visibleCount, setVisibleCount] = useState(6);
  const isLoadingMore = useRef(false);

  // iPad AppleCare Dynamic Data
  const [dbAppleCareRows, setDbAppleCareRows] = useState(() => {
    try {
      const cached = localStorage.getItem('iincept_ipad_applecare_rows_v2');
      if (cached) {
        const parsed = JSON.parse(cached);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      }
    } catch (e) {}
    return DEFAULT_IPAD_APPLECARE_ROWS;
  });
  ...
  const fetchNavSettings = async () => {
    try {
      ...
      if (response.data) {
        // Load iPad AppleCare Pricing Table
        if (response.data.appleCarePricingTables && response.data.appleCarePricingTables.length > 0) {
          const ipadTable = response.data.appleCarePricingTables.find(t => t.categoryKey === 'ipad');
          if (ipadTable) {
            ...
            if (ipadTable.rows && ipadTable.rows.length > 0) {
              const activeRows = ipadTable.rows.filter(r => r.isActive !== false);
        ...
        if (response.data.categoryIconGroups && response.data.categoryIconGroups.length > 0) {
          const ipadGrp = response.data.categoryIconGroups.find(g => g.categoryKey === 'ipad');
          if (ipadGrp && ipadGrp.icons && ipadGrp.icons.length > 0) {
            const activeSub = ipadGrp.icons.filter(d => d.isActive !== false);
            if (activeSub.length > 0) {
              const mapped = activeSub.map(d => {
                const fallback = IPAD_SUB_NAV_ITEMS.find(m => m.name.toLowerCase() === (d.label || '').toLowerCase());
                return {
                  name: d.label,
              ...
              try {
                localStorage.setItem('iincept_sub_items_v2_ipad', JSON.stringify(withCare));
              } catch (e) {}
        ...
        if (response.data.navbarMenuItems) {
          const ipadItem = response.data.navbarMenuItems.find(i => (i.name || '').toLowerCase().includes('ipad'));
          if (ipadItem && ipadItem.dropdownItems && ipadItem.dropdownItems.length > 0) {
            const activeSub = ipadItem.dropdownItems.filter(d => d.isActive !== false);
            if (activeSub.length > 0) {
              const mapped = activeSub.map(d => {
                const fallback = IPAD_SUB_NAV_ITEMS.find(m => m.name.toLowerCase() === (d.label || '').toLowerCase());
                return {
      ...
      console.error('Failed to load ipad sub nav settings:', err);
    }
  };

  const handleBuyAppleCareWhatsApp = (modelObj = {}) => {
    const modelName = modelObj.title || modelObj.model || 'iPad';
    const salePrice = modelObj.salePrice || modelObj.yearly || '₹8,900.00';
  ...
  const handleAddAppleCareToCart = (rowObj, i) => {
    ...
    const numericPrice = parseFloat(priceStr.replace(/[^0-9.]/g, '')) || 8900;
    const image = rowObj.image || getModelImageByName(rowObj.model);
    const itemId = `ac-ipad-${sku || i}`;
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] text-[#1d1d1f] py-8 px-4 sm:px-8 md:px-12 lg:px-16 select-none animate-in fade-in duration-300 relative">
                    ...
                    <img
                      ...
                      onError={(e) => {
                        ...
                        if (lower.includes('care')) {
                          ...
                          e.currentTarget.src = '/ipad_nav/ipad_air.png';
                        }
                      }}
      ...
      {/* Conditionally Render iPad AppleCare Grid OR Product Grid */}
      {(() => {
        const currentTab = searchParams.get('tab') || '';
        const currentSearch = searchParams.get('search') || '';
        const isAppleCareActive = currentTab.toLowerCase() === 'applecare' || currentSearch.toLowerCase().includes('care');
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-xs font-bold text-white">
                                {Object.keys(selectedAppleCareMap).length} iPad AppleCare Plans Selected
                              </div>
